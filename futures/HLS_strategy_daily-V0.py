...
def call_signal(o,bars):
    ...
    f3 = 144
    f4 = 233
    o.idata =o.idata.append(bars[['open','low','high','close']])
    #import pdb
    #pdb.set_trace()
...
def on_dayBar(ctx,bars):
    ...
    o.idata = o.idata.append(bars[['open','low','high','close']][-1])
    t2 =  time.time()
    logging.info("index=%s,symbole=%s,haoshi=%s,initflag=%s,hold=%s"
...
def on_bar(ctx,bars):
    ...
    o = ctx.symbolsdata.get(symbol) #获取代码对应的的对象
    if o.initflag == True:
#        if (index.hour!=9 and index.minute!=0) or (index.hour!=: #时间用叠加的
#            o.idata = o.idata.drop(o.idata.index[-1],axis=0)
#         
